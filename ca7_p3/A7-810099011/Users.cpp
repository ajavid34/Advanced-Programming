...
void Status::reset(){
	...
	en_filter = false; wrong_filter = false;
	change_rate = false; new_rate = false;
	add_rec_to_shelve = false; wrong_add_rec_to_shelve = false;
}
string Chef::show_my_recipe_html(){
	sort_recipes(recipes);
	ostringstream body;
	body << "";
	for (int i = 0; i < recipes.size(); i++)
		body << recipes[i]->glimpse_recipe_html();
    return body.str();
}
string O_User::show_my_shelve_html(){
	sort_shelves_id(shelves);
	ostringstream body;
	body << "";
	for (int i = 0; i < shelves.size(); i++)
		body << shelves[i]->glimpse_shelve_html();
    return body.str();
}