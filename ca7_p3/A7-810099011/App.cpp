...
User* App::create_new_user(string username, string password, string roll){
	if(is_already_signup(username)) 
		throw Bad_Request_Error();
	if (roll == CHEF_KEYWORD) {
		Chef* new_ch = new Chef(username,password);
		chefs.push_back(new_ch);
		return new_ch;
	}
	else {
		O_User* new_us = new O_User(username,password);
		users.push_back(new_us);
		return new_us;
	}
}

string App::get_active_roll(string name){
	O_User* user = find_ousers(name);
	Chef* chef = find_chefs(name);
	if (chef != NULL)
		return CHEF_KEYWORD;
	else 
...
void App::login_user(string name, string pass) {
	current_ouser = find_ousers(name);
	current_chef = find_chefs(name);
	check_login(pass);
}
string App::show_chef_recipe(std::string name){
	ostringstream body;
	Chef* chef = find_chefs(name);
	body << chef->show_my_recipe_html();
	return body.str();
}
void App::add_recipe(string username,string title,string ingredients,string tags,string img_add,string veg,int min_to_ready){
	Chef* chef = find_chefs(username);
	shared_ptr<Recipe> newrecip (make_shared <Recipe>(title, ingredients,tags, img_add,veg,min_to_ready,now_id_recipes,chef));
	recipes.push_back(newrecip);
	chef->new_recipes(newrecip);
	now_id_recipes++;
}
string App::show_recipe_id_html(int id, string username, Status* status){
	O_User* user = NULL;
	Chef* chef = NULL;
	user = find_ousers(username);
	chef = find_chefs(username);
	ostringstream body;
	shared_ptr <Recipe> recipe = find_recipe(id,recipes);
	if (recipe == NULL)
		throw Not_Found_Error();
	body << recipe->show_full_recipe_html(username);
	if (chef != NULL)
		body << chef_show_recipe_page(id);
	else if (user != NULL){
		body << user_show_recipe_page(id);
		if (status->change_rate) {
			body << "<p>&#9989 Change Rate was Successful.</p>";
			status->change_rate = false;
		}
		if (status->new_rate) {
			body << "<p>&#9989 Rating was Successful.</p>";
			status->new_rate = false;
		}
	}
	body << "</body>"
    << "</html>";
...
string App::chef_show_recipe_page(int id){
	ostringstream body;
	body
	<< "<br /> <br />"
	<< "<form action='/delete_recipe' method='get'>"
...
string App::user_show_recipe_page(int id){
	ostringstream body;
	body
	<< "<br /> <br />"
	...
	<< "<input type='hidden' id='custid' name='id' value='" << id << "'>"
	<< "<div class='form-group'>"
  	<< "<input type='number' id='rates' name='rates' min='1' max='5' placeholder='Rate (1 to 5)' , required>"
	<< "<br /> <br />"
	<< "<button type='submit' class='button1'>Rate</button>"  
	<< "  </div> </div> </form>";
	return body.str();
}
void App::delete_recipe_id(string name, int id) {
	Chef* chef = find_chefs(name);
	int index = find_index_recipe(id,recipes);
	if (index == NOT_FOUND_INDEX)
		throw Not_Found_Error();
	chef->remove_recipes(id);
	recipes.erase(index + recipes.begin());
}
string App::show_all_recipes_html(){
	ostringstream body;
	sort_recipes(recipes);
	for (auto x : recipes) 
		if (is_passed_filters(x))
			body << x->glimpse_recipe_html();
	return body.str();
}
void App::set_filters(string type, int min, int max){
	if (type == TIME_KEYWORD)
		time_filter.set_filter(min, max);
	else if (type == RATINGS_COMMAND)
		rate_filter.set_filter(min, max);
	else if (type == VEGETARIAN_KEYWORD){
		veg_filter.set_off();
		veg_filter.set_filter();
	}
	else if (type == NOT_VEGETARIAN_FILTER_KEYWORD){
		veg_filter.set_off();
		veg_filter.set_not_veg();
	}
}
void App::rate_recipe_id(string name,int id, int score, Status* status){
	O_User* user = find_ousers(name);
	shared_ptr<Recipe> recipe = find_recipe(id, recipes);
	try{
		recipe->get_rated(score, user);
		status->new_rate = true;
	}
	catch (Bad_Request_Error& b) {
		recipe->change_rate(score, user);
		status->change_rate = true;
    }
}
string App::show_user_shelves(string name){
	O_User* user = find_ousers(name);
	ostringstream body;
	body << user->show_my_shelve_html();
	return body.str();
}
void App::add_shelve(string username, string title){
	O_User* user = find_ousers(username);
	shared_ptr<Shelve> newshelv(make_shared<Shelve>(title, now_id_shelves, user));
	shelves.push_back(newshelv);
	user->new_shelves(newshelv);
	now_id_shelves++;
}
string App::show_shelve_id_html(int id){
	ostringstream body;
	shared_ptr<Shelve> shel = find_shelve(id,shelves);
	if (shel == NULL)
		throw Not_Found_Error();
	body << shel->show_full_shelve_html();
	return body.str();
}

void App::add_recipe_shelve(string name,int id_recipe, int id_shelve){
	O_User* user = find_ousers(name);
	shared_ptr<Recipe> rec = find_recipe(id_recipe, recipes);
	if (rec == NULL)
		throw Not_Found_Error();
	user->put_shelves_recipes(id_shelve, id_recipe, rec);
}
void App::reset_filters_html(){
	tag_filter.set_off();
	veg_filter.set_off();
	time_filter.set_off();
	rate_filter.set_off();
}
void App::delete_recipe_shelve_html(std::string username,int id_recipe, int id_shelve){
	O_User* user = find_ousers(username);
	user->delete_shelves_recipes(id_shelve,id_recipe);
}